...
(function () {
  ...
  function makeInlineSvgAbstract(params) {
    ...
    const attrClass = [config.replacementClass, iconName ? ''.concat(config.familyPrefix, '-').concat(iconName) : '', widthClass].filter((c) => extra.classes.indexOf(c) === -1).filter((c) => c !== '' || !!c).concat(extra.classes).join(' ');
    const content = {
      children: [],
